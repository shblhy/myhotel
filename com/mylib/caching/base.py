...
from .invalidation import invalidator, flush_key, make_key, byid, cache, get_key_list
...
class CacheMachine(object):
    ...
    def __init__(self, query_string, iter_function, model=None, timeout=None, db='default'):
    ...
    def query_key(self):
        ...
        return make_key(query_db_string, with_locale=False,model=self.model)
...
class CachingQuerySet(models.query.QuerySet):
    ...
    def iterator(self):
        ...
        if self.timeout == NO_CACHE:
            ...
            return iter(CacheMachine(query_string, iterator, self.model, self.timeout, db=self.db))
...
class CachingMixin(object):
    ...
    @classmethod
    def get_cache_keys_list(model_class):
        return get_key_list(model_class)
...
def clear_cache(model_class):
    for i in get_cache_keys_list(model_class):
        cache.set(i,None)
