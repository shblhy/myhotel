/*******************************************************************************
 * 1.urls新增multistatusbar链接，即整条bar跳转链接
 * 2.新增onClick函数
 * 3.若颜色值为false或transparent，则不显示在showValuesInLegend（legend）上
 * 4.新增legend定位
 * 5.levelClass
 * 
 * 
...
(function ($, undefined) {
    ...
    $.widget("ui-marccarre.multistatusbar", {
        ...
        options: {
            ...
            payload: {},
            urls: {},
            colors: ['transparent','#31d645','#ec4232','#d631eb','#e8c12e','#2664d7','#e5e5e5'],
            levelClass: ['level_0','level_1','level_2','level_3','level_4','level_5','level_6'],
            showLegend: true,
            showValuesInLegend: false,
            showValuesInBar: true,
            refreshUrl: null,
            legendPosition:'left',
            legendMarginLeftFromPayload:'',
        ...
        _createAndPopulateWidget: function (totalNumObjects) {
            ...
            var levelClass = this.options.levelClass;
            ...
            for (var key in payload) {
                ...
                var color = colors[i];
                var level = levelClass[i];

                if (value > 0) {
                    var td = bar.addValue((this.options.showValuesInBar ? value : "&nbsp;"), value * objectWidth, color, level);
            ...
            if(this.options.onClick && (typeof this.options.onClick == 'function')){
            	...
            	this.element.click(function(){
            		_this.options.onClick.call(this,_this.options.id)
            	})
            }
            /*--end2--*/
            
            /*--1.urls新增multistatusbar链接，即整条bar跳转链接--*/
            var multistatusbar = 'multistatusbar';
            if(multistatusbar in urls){
            	bar.table.wrap('<a href="' + urls[multistatusbar] + '" target="_blank" ></a>');
            }
            /*--end1--*/
	    	/*--4.新增legend定位--*/
	    	if(this.options.showLegend){
		    	if(this.options.legendPosition == 'right'){
		    		legend.div.css('margin-left', this.options.width - parseInt(legend.div.css('width')));
		    	}
		    	if(this.options.legendMarginLeftFromPayload){
		    		var keys = this.options.legendMarginLeftFromPayload.split(','), marginLeft = 0;
		    		for(key in keys){
		    			if(keys[key]){
		    				marginLeft += payload[keys[key]]*objectWidth;
		    			}
		    		}
		    		legend.div.css('margin-left', marginLeft);
		    	}
		    }
	    	/*--end3--*/
        },
    ...
    function StatusBar(width) {
        ...
        this.addValue = function (value, width, color, level) {
            var td = $("<td class='"+level+"' style='background-color: " + color + "; width:" + width + "px;'>" + value + "</td>");
            this.tr.append(td);
            return td;
        }
    }
