 ...
 *
 *
 * svgMap 5.5.2 - Interactive Map Plugin
 *
 ...
 *
 * You must purchase Regular or Extended license to use svgMap plugin.
    ...
    var mapSVG = function(elem, options) {
        ...
        this.methods = {
            ...
            destroy: function() {
                ...
                this.removeTooltip(_data);   
                _data.$map.removeAttr('style').empty();     
                return _this;
            },
            getData: function() {
                return _data;
            ...
            //$$ btnPre
            btnBack: function() {
                _data.options.backFunction.call(this, _data);
            },
            ...
            panEnd: function(e) {
                ...
                _this.regionClickHandler(e);
                /***12.取消panRegionClickHandler方法绑定的region_clicked***/
            ...
            selectRegion: function(id) {
                ...
                if (_data.options.multiSelect) {
                    ...
                    if (i >= 0) {
                        ...
                        _data.ref[id].attr({
                            'fill': _data.ref[id].default_attr.fill
                        });
            ...
            unhighlightRegion: function(id) {
                ...
                if (_data.ref[id].disabled || (_data.options.multiSelect && $.inArray(id, _data.selected_id) >= 0) || _data.selected_id == id) return false;
            ...
            highlightRegion: function(id) {
                if (_data.isPanning || _data.ref[id].disabled || (_data.options.multiSelect && $.inArray(id, _data.selected_id) >= 0) || _data.selected_id == id)
                    return false;
            ...
            regionClickHandler: function(e) {
                ...
                if (_data.region_clicked && (_data.region_clicked.disabled != true) && Math.abs(_data.pan.dx) < 5 && Math.abs(_data.pan.dy) < 5){

                    _this.selectRegion(_data.region_clicked.name);
                    _this.hidePopover();

                    if (_data.region_clicked.popover)
                        _this.showPopover(e, _data.region_clicked.popover);

                    if (_data.options.onClick)
                        return _data.options.onClick.call(_data.region_clicked, e, _this);

                    if (touchDevice && _data.region_clicked.attrs.href)
                        window.location.href = _data.region_clicked.attrs.href;
            ...
            renderSVGPath: function(item, parentTransform, parentStyle) {
                var rObj = _this.initRaphaelObject(_data.R.path($(item).attr('d')), item, parentTransform, parentStyle);
            ...
            renderSVGPolygon: function(item, parentTransform, parentStyle) {
                var points = $.trim(item.getAttribute('points')).replace(/ +(?= )/g, '').split(/\s+|,/);
                var x0 = points.shift(),
                    y0 = points.shift();
            ...
            renderSVGPolyline: function(item, parentTransform, parentStyle) {
                ...
                _this.initRaphaelObject(_data.R.path(pathdata), item, parentTransform, parentStyle);
                return rObj;
            },
            ...
            initRaphaelObject: function(rObj, svgItem, parentTransform, parentStyle) {
                ...
                var style = $.extend(true, {}, parentStyle, _this.styleSVG2Raphael(svgItem));
                rObj.attr(style);
                var transform = _this.transformSVG2Raphael(svgItem) + parentTransform;
                rObj.transform(transform);
                return rObj;
            },
            styleSVG2Raphael: function(svgItem) {

                var styles = {};
                var attrs = $(svgItem).get(0).attributes;
                var style_attrs = ["fill", "fill-opacity", "opacity", "font", "font-name", "font-family", "font-size", "font-weight", "stroke-lincap", "stroke-linejoin", "stroke-miterlimit", "stroke-opacity", "stroke-width"];

                if ($(svgItem).attr('style')) {
                    var _styles = $(svgItem).attr('style').split(';');
                    $.each(_styles, function(i, val) {
                ...
                if (styles['font-size']) {
                    styles['font-size'] = parseInt(styles['font-size']);
                }

                return styles;

                //raphaelItem.attr(styles);
            },
            ...
            regionAdd: function(_item) {
                ...
                _data.ref[name].default_attr['stroke'] = _item.attr('stroke') || 'none';

                if (_data.ref[name].default_attr['stroke'])
                    _data.ref[name].default_attr['stroke-width'] = parseFloat(_item.attr('stroke-width')); //styles.getPropertyValue('stroke-width') || $(_item).attr('stroke-width');
                ...
                if (_data.options.regions[name]) {
                    //$$3.添加deep,froms属性2>>>>>>>如果传入的json对象regions中包含有deep属性，则赋值，否，则赋缺省值_data.options.deep;

                    if (_data.options.regions[name].forms)
                        _data.ref[name].forms = _data.options.regions[name].forms;

                    if (_data.options.regions[name].deep != undefined)
                        _data.ref[name].deep = _data.options.regions[name].deep;
                    else
                        _data.ref[name].deep = _data.options.deep;
                    //<<<<<<<end
                    ...
                    /***8."attr": {"fill": "1"}，数字1以上为级别，颜色与levelColorItems=[{color:'#80ba00',text:'100~80'}]对应，索引0用作基本色****/
                    if (_data.options.levelColorItems && _data.options.levelColorItems.length) {
                        var index = _data.ref[name].default_attr['fill'];
                        if (typeof index == 'number') {
                            _data.ref[name].default_attr['fill'] = _data.options.levelColorItems[index].color;
                        } else if ((_data.ref[name].default_attr['fill'] != _item.attr('fill')) && (_data.ref[name].default_attr['fill'] != 'none') && (_data.ref[name].default_attr['fill'] != _data.options.colors.base)) {
                            _data.ref[name].default_attr['fill'] = _data.options.levelColorItems[0].color;
                        }
                    }
                    /***end9***/
            ...
            setLevelColor: function() {
                if (_data.options.levelColorItems && _data.options.levelColorItems.length && _data.options.showLevelColor) {
                    // 添加级别颜色
                    ...
                    colorBox.append($('<div/>').text(_data.options.levelColorTitle).css({
                        'height': '14px',
                        "line-height": '14px',
                        "margin-bottom": '2px',
                        "margin-top": '2px'
                    ...
                    $.each(_data.options.levelColorItems, function(i, j) {
                        if (j.text) {
                            var colorBlock = $('<span/>').css(cssColorBlock).css('backgroundColor', j.color);
                            var item = $('<span/>').css(cssItem);
                            item.append(colorBlock).append(j.text);
                            colorBox.append(item);
                        }
                    });

                    _data.levelColorItems = colorBox;

                    _data.$map.append(_data.levelColorItems);
                }
            },
            /*end*/
            setZoom: function(on) {
                if (on) {
                    ...
                    _data.$map.bind('mousewheel.svgmap', function(event, delta, deltaX, deltaY) {
                        var d = delta > 0 ? 1 : -1;
                        _this.zoom(d);
                        return false;
                    });
                    ...
                    if (_data.options.zoomButtons.show) {
                        ...
                        var btnZoomIn = $('<img title="放大" src="data:image/png;base64," id="btnZoomIn"/>').on('click', function(e) {
                            e.stopPropagation();
                            _this.zoomIn();
                        }).css(cssBtn);

                        var btnZoomOut = $('<img title="缩小" src="data:image/png;base64," id="btnZoomOut"/>').on('click', function(e) {
                            e.stopPropagation();
                            _this.zoomOut();
                        }).css(cssBtn);

                        /**属于多层deep == true && 当前地区id != 顶层地区id 时 显示向上按钮**/
                        if ((_data.options.deep == true) && (_data.options.currentRegionId != _data.options.topRegionId)) {
                            //var btnBack = $('<img title="返回" src="data:image/png;base64," id="'+_data.options.currentRegionId+'"/>').on('click', function(e){
                            var btnBack = $('<img title="返回" src="data:image/png;base64," id="' + _data.options.currentRegionId + '"/>').on('click', function(e) {
                                e.stopPropagation();
                                _this.btnBack();
                            }).css(cssBtn);
                        }

                        buttons.append(btnZoomOut).append(btnZoomIn).append(btnBack).css({
                            position: 'absolute',
                            top: '15px',
                            width: '16px',
                            cursor: 'pointer'
                    ...
                    if (_data.options.zoom)
                        _data.$map.unbind('mousewheel.svgmap');
            ...
            setSize: function(width, height, responsive) {
                ...
                if (_data.options.responsive) {
                    ...
                    $(window).bind('resize.svgmap', function() {
            ...
            initTooltip: function(_data) {
                if(_browser.ie && _browser.ie.old){
                    ...
                    _data.$map.mousemove(function(e) {
                        _data.mapTip.css('left', $(e.target).offset().left + $(window).scrollLeft() - _data.mapTip.width() - 20).css('top', $(e.target).offset().top + $(e.target).height()/2 - _data.mapTip.height()/2);
                    });

                    _this.showTip = function(name) {
                    ...
                    _this.showTip = function(name) {
                        if (_data.ref[name].tooltip) {
                            _data.$map.append($('<div style="display:none;" id="inline_' + name + '"><div class="table table-striped overflow-y">' + _data.ref[name].tooltip + '</div></div>'));
                            Tipped.create($('#' + name), 'inline_' + name, _data.options.tippedOptions).show();
                        }
                    };
                }
            },
            removeTooltip: function(_data) {
                if(_browser.ie && _browser.ie.old){                              
                    //清除tooltip
                    $('.map_tooltip').remove();
                }else{
                    for(var index in _data.ref)
                        Tipped.remove($('#' + index));
                }
            },
            ...
            init: function(opts, elem) {
                ...
                _data.$map.css({
                    ...
                    /**14**
                             'height': '100px',
                             **/
                    'height': _data.$map.parent().height(),
                    /**end**/
                    'position': 'relative'
                });
                ...
                $.ajax({
                    ...
                    success: function(xmlData) {
                        ...
                        if (_browser.ie && _browser.ie.old) {
                            ...
                            <<<<<<<*/
                                $(window).on('resize', function() {
                                    var parent_width = parseInt(_data.$map.parent().css('width'));
                                    _data.$map.css({
                            ...
                            /**7**/
                            if (_data.options.responsive || $.browser.chrome) {
                                /**end7**/
                                $(window).on('resize', function(e) {
                                    /**5**/
                                    var parent_width = parseInt(_data.$map.parent().css('width'));
                                    //_data.$map.css('max-width',parent_width+'px');
                                    _data.$map.css({
                                        'max-width': parent_width,
                                        'width': parent_width,
                                        'overflow': 'hidden'
                        ...
                        (function traverse(node, parentTransformString, parentStyle) {
                            ...
                            if (children.length && !$(node).is('text')) {
                                ...
                                $(node).children().each(function(i, n) {
                                    parentStyle = $.extend(true, {}, parentStyle, _this.styleSVG2Raphael(n));
                                    traverse($(n), parentTransformString, parentStyle);
                                });
                        ...
                        if (!touchDevice) {
                            ...
                            funcStr += 'methods.showTip(this.name, _data);';
                            if (_data.options.mouseOver)
                                funcStr += 'return options.mouseOver.call(this, e, methods);';

                            _this.mouseOverHandler = new Function('e, methods, options, _data', funcStr);

                            // 2. MouseOut
                            funcStr = '';
                            funcStr += 'methods.unhighlightRegion(this.name);';
                            funcStr += 'methods.hideTip(this.name, _data);';
                            if (_data.options.mouseOut)
                                funcStr += 'return options.mouseOut.call(this, e, methods);';
                            _this.mouseOutHandler = new Function('e, methods, options, _data', funcStr);
                        }
                            ...
                            if(_browser.ie && _browser.ie.old){
                                _data.RMap
                                    .mousemove(function(e) {
                                        _this.mouseOverHandler.call(this, e, _this, options, _data);
                                    })
                                    .mouseout(function(e) {
                                        _this.mouseOutHandler.call(this, e, _this, options, _data);
                                    });                                
                            }else{                                
                                _data.RMap
                        ...
                        _data.$map.data('svgMap',_data);
                        loading.hide();
                        /*$$ 2.规避某些浏览器地图偏离中心位置，模拟放大缩小操作，触发 */
