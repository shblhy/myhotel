/***
 *getValues返回字符串，获取选中选项id
 *getTexts返回字符串，获取选中选项text
 *
 *
 *
 *
 ...
 * parser
 *
 *1a.懒加载子节点选中状态根据展开节点判断
 *2a.当checked===null为半选状态
 *
 */
(function($) {
	$.parser = {
		...
		plugins: ["draggable", "droppable", "resizable", "pagination", "tooltip", "linkbutton", "menu", "menubutton", "splitbutton", "progressbar", "tree", "selecter", "treeselecter", "combogrid", "numberbox", "validatebox", "searchbox", "numberspinner", "timespinner", "calendar", "datebox", "dateinput", "slider", "layout", "panel", "datatable", "propertygrid", "treetable", "tabs", "foldpanel", "window", "dialog"],
		parse: function(_2) {
			var aa = [];
			for (var i = 0; i < $.parser.plugins.length; i++) {
				var _3 = $.parser.plugins[i];
				var r = $(".uiWidgets-" + _3, _2);
				if (r.length) {
					if (r[_3]) {
						r[_3]();
					} else {
...
(function($) {
	...
	function _c(_d) {
		...
		$(_d).unbind().bind("mouseover", function(e) {
			...
			if (tt.hasClass("tree-hit")) {
				 ...
				 */
				if (tt.hasClass("tree-checkbox") || tt.hasClass("tree-title")) {
					_39(_d, _11[0], !_11.find(".tree-checkbox1").length);
					_e.onClick.call(_d, _14(_d, _11[0]));
					/*--end--*/
					/*-增加选中class-*/
					$(this).find('.tree-node-active').removeClass('tree-node-active');
					_11.addClass("tree-node-active");
					/*-end-*/
					return false;
				} else {
					/*--3.禁止点击空白处关闭panel --
	...
	function _15(_16) {
		...
		/***注释掉功能***_17.draggable("disable");*/
		_17.css("cursor", "default");
	};
	...
	function _18(_19) {
		...
		_1c.find("div.tree-node").draggable({
			...
			onStartDrag: function() {
				...
				if (_1f.id == undefined) {
					_1f.id = "uiWidgets_tree_node_id_temp";
					_c2(_19, _1f);
				}
			...
			onStopDrag: function() {
				...
				if (_20 && _20.id == "uiWidgets_tree_node_id_temp") {
					_20.id = "";
					_c2(_19, _20);
				}
	 ...
	 */
	function _52(_53, ul, _54, _55, _74) {
	/***end1a***/
			...
			for (var i = 0; i < _5c.length; i++) {
				...
				if (_56.checkbox) {
					if (_56.onlyLeafCheck) {
						...
						if (_5e.checked) {
							$("<span class=\"tree-checkbox tree-checkbox1\"></span>").prependTo(_5f);
							_57.push(_5f[0]);//保存选中的节点
						} else {
							$("<span class=\"tree-checkbox tree-checkbox0\"></span>").prependTo(_5f);
						}
						*/
						if (_74 && _74.checked) {//当传入父级，且选中，则选中
							$("<span class=\"tree-checkbox tree-checkbox1\"></span>").prependTo(_5f);
							_57.push(_5f[0]);//保存选中的节点
						} else if(_74 && !_74.checked){//当传入父级，且不选中，则不选中
							$("<span class=\"tree-checkbox tree-checkbox0\"></span>").prependTo(_5f);
						} else if(_5e.checked){//checked为true，则选中
							$("<span class=\"tree-checkbox tree-checkbox1\"></span>").prependTo(_5f);
							_57.push(_5f[0]);//保存选中的节点
						} else if(_5e.checked === null){//checked为null，则半选中泰
							$("<span class=\"tree-checkbox tree-checkbox2\"></span>").prependTo(_5f);
						}else{//checked未定义或等同于false，则不选
							$("<span class=\"tree-checkbox tree-checkbox0\"></span>").prependTo(_5f);
						}
						/**end1a - 2a**/
	...
	function _6f(_70, ul, _71, _72) {
		 ...
		 */
		var _76 = $(ul).prev().children("span.tree-hit");
		/***end2***/
		...
		var _77 = _73.loader.call(_70, _71, function(_78) {
			...
			_52(_70, ul, _78);
		 */
		var _77 = _73.loader.call(_70, _71, function(_78, _74) {
			_76.removeClass("tree-loading");
			_52(_70, ul, _78, null, _74);
		/***end1a***/
			if (_72) {
				_72();
			}
	...
	function _a4(_a5, _a6) {
		...
		$(_a5).find(_a7).each(function() {
			...
			if (!$item.closest('ul').prev('div').find('.tree-checkbox1').length) {
				_a8.push(_14(_a5, $item[0]));
			}
			/*--end--*/
		});
		return _a8;
	...
	function _aa(_ab, type) {
		var _ac = _a4(_ab);
		if (_ac.length) {
			var value = [];
			if (type == 'text') {
				for (index in _ac) {
					value.push(_ac[index].text);
				}
				value = value.join(',');
			} else {
				for (index in _ac) {
					value.push(_ac[index].id);
				}
			}
			return value;
		} else {
	...
	$.fn.tree.methods = {
		...
		/**1.**/
		setChecked: function(jq, _f8) {
			return jq.each(function() {
				var node = $(this).find('div[node-id=' + _f8 + ']');
				_39(this, node, true);
			});
		},
		/**end1**/
	...
	$.fn.tree.defaults = {
		...
		**/
		loader: function(_109, _10a, _10b, _74) {
		/**1a**/
