/*
 * 1.注释nHidden.style.left - this.dom.collection.style.width
 * 2.添加清空按钮
 * 3.批量操作与列操作分割
 * 4.'aoColumns增加sShowC':false 不显示筛选按钮
 * 
 * 
 * 
 * 
	...
	ColVis.prototype = {
		...
		* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
			...
			**/
			...
			$(this.s.dt.oInstance).bind('column-reorder', function(e, oSettings, oReorder) {
				for( i = 0, iLen = that.s.aiExclude.length; i < iLen; i++) {
					that.s.aiExclude[i] = oReorder.aiClearMapping[that.s.aiExclude[i]];
				}
		...
		"_fnApplyCustomisation" : function() {
			...
			/***2***/
			if( typeof oConfig.bShowClear != 'undefined') {
				this.s.bShowClear = oConfig.bShowClear;
			}

			if( typeof oConfig.sShowClear != 'undefined') {
				this.s.sShowClear = oConfig.sShowClear;
			}
			/***end2***/
		...
		"_fnAddButtons" : function() {
			var nButton, sExclude = "," + this.s.aiExclude.join(',') + ",";
			...
			this.dom.collection.appendChild(line);
			/***end3***/
			...
			/***2***/
			if(this.s.bShowClear) {
				nButton = this._fnDomShowClearButton();
				nButton.className += " ColVis_ShowClear";
				this.dom.buttons.push(nButton);
				this.dom.collection.appendChild(nButton);
			}
			/***end2***/
		...
		"_fnDomRestoreButton" : function() {
			var that = this, nButton = document.createElement('button'), nSpan = document.createElement('span');

			/**
			nButton.className = !this.s.dt.bJUI ? "ColVis_Button TableTools_Button" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			**/
			nButton.className = !this.s.dt.bJUI ? "btn btn-success margin-right-small" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			/****/
		...
		"_fnDomShowAllButton" : function() {
			...
			nButton.className = !this.s.dt.bJUI ? "ColVis_Button TableTools_Button" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			**/
			nButton.className = !this.s.dt.bJUI ? "btn btn-primary margin-right-small" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			/****/
			nButton.appendChild(nSpan);
		...
		/***2***/
		"_fnDomShowClearButton" : function() {
			var that = this, nButton = document.createElement('button'), nSpan = document.createElement('span');

			/**
			nButton.className = !this.s.dt.bJUI ? "ColVis_Button TableTools_Button" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			**/
			nButton.className = !this.s.dt.bJUI ? "btn btn-info margin-right-small" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			/****/
			nButton.appendChild(nSpan);
			$(nSpan).html('<span class="ColVis_title">' + this.s.sShowClear + '</span>');

			$(nButton).click(function(e) {
				for(var i = 0, iLen = that.s.abOriginal.length; i < iLen; i++) {
					if(that.s.aiExclude.indexOf(i) === -1) {
						that.s.dt.oInstance.fnSetColumnVis(i, false, false);
					}
				}
				that._fnAdjustOpenRows();
				that.s.dt.oInstance.fnAdjustColumnSizing(false);
		...
		"_fnDomColumnButton" : function(i) {
			var that = this, oColumn = this.s.dt.aoColumns[i], nButton = document.createElement('button'), nSpan = document.createElement('span'), dt = this.s.dt;

			/**4**/
			if(oColumn.sShowC === false){
				$(nButton).css({
					'display':'none'
				});
				that.s.aiExclude.push(i);
			}
			/**end**/

			nButton.className = !dt.bJUI ? "ColVis_Button TableTools_Button" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			nButton.appendChild(nSpan);
			var sTitle = this.s.fnLabel === null ? oColumn.sTitle : this.s.fnLabel(i, oColumn.sTitle, oColumn.nTh);
			$(nSpan).html('<span class="ColVis_radio"><input type="checkbox"/></span>' + '<span class="ColVis_title">' + sTitle + '</span>');
		...
		"_fnDomBaseButton" : function(text) {
			var that = this, nButton = document.createElement('button'), nSpan = document.createElement('span'), sEvent = this.s.activate == "mouseover" ? "mouseover" : "click";
			
			/**
			nButton.className = !this.s.dt.bJUI ? "ColVis_Button TableTools_Button" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			**/
			nButton.className = !this.s.dt.bJUI ? "" : "ColVis_Button TableTools_Button ui-button ui-state-default";
			/****/
			/**简化结构
			nButton.appendChild(nSpan);
		...
		"_fnDomCollection" : function() {
			...
			nHidden.className = !this.s.dt.bJUI ? "ColVis_collection TableTools_collection" : "ColVis_collection TableTools_collection ui-buttonset ui-buttonset-multi";
		...
		"_fnCollectionShow" : function() {
			...
			var iWinHeight = $(window).height(), iDocHeight = $(document).height(), iWinWidth = $(window).width(), iDocWidth = $(document).width();
			...
			/****/
			var btnPaddingLeft = parseInt($(this.dom.button).css('padding-left'));
			var btnPaddingRight = parseInt($(this.dom.button).css('padding-right'));
			var iDivXRight = $(document).width() - ($(this.dom.button).width() + btnPaddingLeft + btnPaddingRight + parseInt(oPos.left, 10));
			
			if(iDivX < iDivXRight)
				nHidden.style.left = iDivX + "px";
			else
				nHidden.style.right = iDivXRight + "px";
			/****/
			...
			if(!this.s.bCssPosition) {
				//nHidden.style.left = this.s.sAlign == "left" ? iDivX + "px" : (iDivX - $(nHidden).outerWidth() + $(this.dom.button).outerWidth()) + "px";
