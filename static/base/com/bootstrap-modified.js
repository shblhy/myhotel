...
(function($) {
  ...
  var MyCollapse = function(element, options) {
    this.$element = $(element);
    //通过构造函数访问静态属性defaults
    this.options = $.extend(this.constructor.defaults, options);

    this.$element.find('> div.active').next('div').show().siblings('div[class*=panel-body]').hide();
    //标题事件绑定
    this.$element.on('click', '> div[class*=panel-heading], > div[class*=panel-footer]', function() {
      var $this = $(this);
      if ($this.hasClass('active')) {
        return;
      }
      $this.next('div').show().siblings('.active').removeClass('active').next('div').hide();
      $this.addClass('active');
    })
    this.$head = this.$element.children('div.panel-heading');
  }
  ...
  MyCollapse.prototype = {
    ...
    show: function(index) {
      this.$element.find('> div.active').removeClass('active');
      $(this.$head.get(index)).addClass('active').next('div').show().siblings('div[class*=panel-body]').hide();
    }
  }
  ...
  //插件定义  
  $.fn.myCollapse = function(options, fnOption) {
    return this.each(function() {
      var $this = $(this),
        data = $this.data('collapse');
      //判断是否有初始化
      if (!data) {
        $this.data('collapse', (data = new MyCollapse(this, options)));
      }
      //调用函数
      if (typeof options == 'string' && fnOption) {
        data[options](fnOption);
      };
    });
  }

  /*插件冲突回滚
  $.fn.collapse.noConflit = function() {
  $.fn.collapse = old;
  return this;
  }
  */
   ...
   *
   * 1.主动，全局绑定
   * $('body').on('click','[data-bootstrap="collapse"]');
   *
   * 2.被动，调用
   * $(target).collapse();
   */
