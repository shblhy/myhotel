 ...
 *
 * bug:
 * 1.调换顺数 ： onShowStep 未return true 干扰 noForwardJumping
 *
 *
 * 
 * 
...
function SmartWizard(target, options) {
    ...
    this.contentWidth = 0;
    this.msgBox = $('<div class="alert"><span class="content"></span><button data-dismiss="alert" class="close" type="button">×</button></div>');
    this.elmStepContainer = $('<div></div>').addClass("stepContainer");
    this.loader = $('<div>Loading</div>').addClass("loader");
    this.buttons = {
        /**1**
        next : $('<a>'+options.labelNext+'</a>').attr("href","#").addClass("next"),
        previous : $('<a>'+options.labelPrevious+'</a>').attr("href","#").addClass("previous"),
        finish  : $('<a>'+options.labelFinish+'</a>').attr("href","#").addClass("finish")
        **/
        next : $('<a href="javascript:void(0)" class="'+options.classNext+'">'+options.labelNext+'</a>'),
        previous : $('<a href="javascript:void(0)" class="'+options.classPrevious+'">'+options.labelPrevious+'</a>'),
        finish  : $('<a href="javascript:void(0)" class="'+options.classFinish+'">'+options.labelFinish+'</a>')
        /**end1**/
    };
...
/**2**/
        if ($this.options.buttons){
            for(var index in $this.options.buttons)
                if($this.options.buttons[index].text)
                    elmActionBar.prepend($('<a href="javascript:void(0)"/>').addClass($this.options.buttons[index].class).text($this.options.buttons[index].text).click(function(){
                        $this.options.buttons[index].click.call($this, this);
                    }))
        }
/**end2**/
...
// Default Properties and Events
    $.fn.smartWizard.defaults = {
        ...
        labelFinish:'完成',
/**1**/
        classNext:'btn next btn-primary',
        classPrevious:'btn previous',
        classFinish:'btn finish btn-success',
/**end1**/
/**3**/
        buttons:[{
          "class":"",//除了类型样式外追加样式
...
/**end3**/
        ...
        ajaxType: "POST",
        onLeaveStep: function($title, fromTo){
            console.log('onLeaveStep')
            //console.log(fromTo.fromStep)
            //console.log(fromTo.toStep)
            //console.dir($title)
            return true;
        }, // triggers when leaving a step
        onShowStep: function($title, fromTo){
            console.log('onShowStep')
            //console.log(fromTo.fromStep)
            //console.log(fromTo.toStep)
            //console.dir($title)
        },  // triggers when showing a step
        onFinish: function($titles, fromTo){//点击完成按钮时触发
