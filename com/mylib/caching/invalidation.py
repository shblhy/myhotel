...
def make_key(k, with_locale=True,model=None):
    ...
    if model:
        key_set=get_key_list(model)
        key_set.add(result)
        cache.set(CACHE_PREFIX+'_'+model.__name__,key_set)
...
def clear_key_list(model):
    for i in get_key_list(model):
        cache.set(i,None)
